...
To test the sensitivity of the prior on the estimates of $\beta$, we ran the model under a normal prior distribution (mean = 0, scale $\sigma$ = 1), $\boldsymbol{\beta} \sim \mathrm{Normal}(0, 1)$. The occupancy estimates were only slightly different ($\Delta$ median = 0--0.01), and the ranking of the detection scenarios was the same (Table S5).
...
Table S5. Model estimates of occupancy at each site ($\psi_h$), regression coefficients ($\beta$), and detection probability of each approach scenario ($\pi$) using a normal prior distribution for $\boldsymbol{\beta}$ ($\boldsymbol{\beta} \sim$ Normal(0,1)). $\pi_1 =$ F-POD data with KERNO-F classifier, $\pi_2 =$ continuous ST data with whistle detector, $\pi_3 =$ duty sampled (15/30 min) ST data with whistle detector, $\pi_4 =$ duty sampled (15/30 min) ST data manually browsed.
