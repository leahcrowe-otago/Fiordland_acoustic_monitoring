...
daily_det_rate_table%>%
  ...
  add_header_above(c(" " = 4, "Occupancy ($\\psi$)" = 3)) %>% 
  kableExtra::kable_classic(full_width = F, html_font = "Cambria")

```

\newpage
Table 2. Detection probability ($\pi$) for the different approach scenarios. ST = SoundTrap, FPOD = FPOD deployment, and BOTH = dual deployment of a SoundTrap and an FPOD at the same location.
...
pi%>%
  ...
  mutate(Device = c("ST", "FPOD", "ST", "ST"),
    ...
    Detector = c("Manual browsing", "FPOD software", "FinFinder", "FinFinder"))%>%
  dplyr::rename("Parameter" = "Fiord_recorder")%>%
  dplyr::select(Parameter, Device, Sampling, Detector, everything())%>%
  kableExtra::kable(booktabs = T, align = "c") %>%
  add_header_above(c(" " = 4, "Scenario detection probability ($\\pi$)" = 3)) %>% 
  kableExtra::kable_classic(full_width = F, html_font = "Cambria")
