...
Fig. 2. Daily detected acoustic presence of bottlenose dolphins (green vertical lines) at each deployment site organized from north to south. The acoustic detection approach ($\pi$) is $\pi_1$ for all F-POD collected data, and for SoundTrap (ST) collected data, $\pi_2$ is continuously recorded data processed with the whistle detector, $\pi_3$ (blue) is duty sampled data (15/30 min) processed with the whistle detector, and $\pi_4$ (orange) is duty sampled data (15/30 min) that were manually browsed for all calls types due to sensitivity issues with the hydrophone. The grey shaded areas are times when the recorder was not active. The vertical red lines indicate when the gear was deployed/retrieved.
